...
if __name__ == '__main__':
    ...
    for dataset in ['static11', 'assist17', 'assist09', 'eedi']:

        with open(f'outputs/ReKT_concept_{dataset}_output.txt', 'w') as file:
            # 遍历列表中的嵌套列表并写入文件
            ...
            for now_step in range(5):
                ...
                model = ReKT_concept(pro_max, skill_max, d, p)
                ...
                for epoch in range(200):
                    ...
                    if valid_auc > best_auc:
                        ...
                        best_auc = valid_auc
                        best_acc = valid_acc
                        torch.save(model.state_dict(), f"./models/ReKT_concept_{dataset}_{now_step}_model.pkl")
                        state['auc'] = valid_auc
                        state['acc'] = valid_acc
                        state['loss'] = valid_loss
                        torch.save(state, f'./state_ckpt/ReKT_concept_{dataset}_{now_step}_state.ckpt')

                    if one_p >= patience:
                        break

                model.load_state_dict(torch.load(f'./models/ReKT_concept_{dataset}_{now_step}_model.pkl'))
            ...
            print(f'*******************************************************************************')
            print(f'*******************************************************************************')
            print(f'{dataset}_final_avg_acc: {avg_acc:.4f}, {dataset}_final_avg_auc: {avg_auc:.4f}')
            print(f'*******************************************************************************')
            print(f'*******************************************************************************')
            print(f'*******************************************************************************')
            print(f'*******************************************************************************')
