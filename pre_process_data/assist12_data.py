from ASSIST12.assist12_utils import *
import os
import pandas as pd
import numpy as np
import time
...
class DataProcess:
    def __init__(self):
        ...
        self.question_id_dict = load_dict("./encode/question_id_dict.txt")
        self.skill_id_dict = load_dict("./encode/skill_id_dict.txt")
        self.template_id_dict = None
        self.type_id_dict = None
        self.train_user_id_dict = None
        self.quesID2skillIDs_dict = {}

    def process_csv(self, isRemoveEmptySkill=True, isRemoveMulSkill=False, isRemoveScaffold=True, min_iter_num=10):
        print("### processing data ###")
        # read original csv file
        df = pd.read_csv(self.data_path, encoding="ISO-8859-1")
        count("originally", df)

        # 1. remove duplicated rows by 'problem_log_id
        df.drop_duplicates(subset=['problem_log_id'], keep='first', inplace=True)
        count('After removing duplicated rows', df)

        # 2. sort records by 'problem_log_id' in ascending order
        df.sort_values(by=['problem_log_id'], ascending=True, inplace=True)
    ...
    def generate_user_sequence(self, df, seq_file):
        ...
        for ui in ui_df:
            ...
            tmp_inter.sort_values(by=['problem_log_id'], ascending=True, inplace=True)
    ...
    def build_ques_interaction_graph(self, train_df, test_df):
        ...
        for ques in self.question_id_dict.keys():
            ...
            tmp_type = tmp_df_0['problem_type']
        ...
        save_graph(ques_template_set, os.path.join(self.save_folder, 'graph', 'ques_template.csv'),
                   ['ques', 'template'])
    ...
    def build_stu_interaction_graph(self, train_df):
        ...
        for stu in self.train_user_id_dict.keys():  # traverse all students in train dataset
            ...
            tmp_df.sort_values(by=['problem_log_id'], ascending=True, inplace=True)
            ...
            for index, row in tmp_df.iterrows():
                ...
                stu_ques_set.add((stuID, quesID, correct, timePoint, attempt_count, ms_first_response))
                timeStep += 1
...
if __name__ == '__main__':
    t = time.time()
    dataPath = "D:/ZJP/ASSIST12/2012-2013-data-with-predictions-4-final.csv"
    saveFolder = './'
    ...
    trainDF = pd.read_csv(os.path.join(saveFolder, 'train_test', 'train_df.csv'))
    testDF = pd.read_csv(os.path.join(saveFolder, 'train_test', 'test_df.csv'))
