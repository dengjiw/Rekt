...
class DataProcess:
    def __init__(self, min_inter_num=3):
    ...
    def process_csv(self, isRemoveEmptySkill=True, isRemoveMulSkill=False):
        ...
        if isRemoveEmptySkill:
            ...
            df['tags'].fillna(value='UNK', inplace=True)

        # 2. remove or keep records with multiple-skill question
        if isRemoveMulSkill:
            df = df[~df['tags'].str.contains(';')]
    ...
    def generate_user_sequence(self, df, seq_file):
        ...
        for ui in ui_df:
            ...
            tmp_inter.sort_values(by=['timestamp'], ascending=True, inplace=True)
            tmp_problems = list(tmp_inter['question_id'])
            tmp_skills = list(tmp_inter['tags'])
    ...
    def build_ques_interaction_graph(self, train_df, test_df):
        ...
        for ques in self.question_id_dict.keys():
            ...
            tmp_skills = [ele for ele in str(tmp_df_0['tags']).split(';')]
            for s in tmp_skills:
    ...
    def build_stu_interaction_graph(self, train_df):
        ...
        num_train_stu, num_skill = len(self.train_user_id_dict), len(
            [s for s in self.skill_id_dict.keys() if ';' not in str(s)])
        ...
        for stu in self.train_user_id_dict.keys():  # traverse all students in train dataset
            ...
            tmp_df.sort_values(by=['timestamp'], ascending=True, inplace=True)

            # # build stu-skill graph, using combined skills
            # for skill in tmp_df['tags'].unique():
            #     skillID = self.skill_id_dict[skill]
            #     skill_df = tmp_df[tmp_df['tags'] == skill]
        ...
        names = ['stu', 'ques', 'correct', 'timePoint', 'elapsed_time']
...
if __name__ == '__main__':
    t = time.time()
    dataPath = "D:/ZJP/EdNet/ednet.csv"
    saveFolder = './'
